...
public:
      ...
      bool hasGuessed(const string& guess) const;         // Checks if the player has already guessed a letter
      void loseAttempt();                                 // Decreases the number of remaining attempts
      int getAttemptsRemaining() const;                   // Returns the number of attempts remaining
      const set<char> getGuessedLetters() const;                // Returns the letters guessed by the player
};